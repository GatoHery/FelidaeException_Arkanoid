    ...
    public partial class GamePlatform : Form
    {
        ...
        private void GamePlatform_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                //Setting attributes for PictureBox hearts
               // hearts = new PictureBox();
               // hearts.Width = hearts.Height = 60;
               // hearts.BackgroundImage = Image.FromFile("../../Textures/Heart.png");
               // hearts.BackgroundImageLayout = ImageLayout.Stretch;
        ...
        private void LoadHearts()
        {
            try
            {
                ...
                cpb = new CustomPictureBox[ay, ax];
                ...
                for (int i = 0; i < ay; i++)
                {
                    for (int j = 0; j < ax; j++)
                    {
                        ...
                        cpb[i, j].Height = PlatformH;
                        cpb[i, j].Width = PlatformW;


                        //Position from Left, Position from Top
                        cpb[i, j].Left = j * PlatformW - 50;
                        cpb[i, j].Top = i * PlatformH + 20;

                        //If the value from i = 0, then put on the route from the brick pictureBox
                        cpb[i, j].BackgroundImage = Image.FromFile("../../Textures/Heart.png");
